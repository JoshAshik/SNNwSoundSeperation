"""
two_speaker_train_v8.py — v8 training: 2-speaker PIT SNN-TasNet on Libri2Mix.

Architecture: SNNTasNet v7 (sep_model_v7.py) — unchanged from v7
...
v8 changes vs v7:
  ...
  - freeze_epochs = 0    (model already trained end-to-end; no warm-up phase needed)
  - LR reset to 1.5e-4   (v7 ended at 1e-6 — fresh cosine schedule over 200 epochs)
  - Three-group AdamW from epoch 1: sep+dec at lr, encoder at lr_finetune
  - Starting checkpoint: checkpoints_v7/best_2spk.pt (best val=-1.16 dB from v7)

Dataset:      LibriMixDataset — pre-generated mix_clean / s1 / s2 wav files
              train-100 (13,900 pairs) + dev (3,000 pairs) on ARC BeeGFS

Resume:       --resume loads checkpoints_v8/best_2spk_latest.pt
SLURM:        slurm_v8_twospeaker.sh (self-resubmitting, 6h wall time)

Usage (smoke test — local, if librimix data present):
    python3 two_speaker_train_v8.py --n_epochs 2 --batch_size 4 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v8_twospeaker.sh
"""
...
DEFAULTS = dict(
    ...
    n_layers          = 3,
    n_speakers        = 2,
    dropout           = 0.5,    # v8: raised from v7's 0.3 to close train-val gap
    snn_chunk         = 100,
    decoder_refine    = 3,
    decoder_groups    = 8,
    # ── training ──────────────────────────────────────────────────────────────
    ...
    lr_finetune       = 5e-5,
    weight_decay      = 1e-4,
    freeze_epochs     = 0,      # v8: no freeze phase — model already trained end-to-end
    val_every         = 5,
    grad_clip         = 5.0,
    ema_decay         = 0.999,
    seed              = 42,
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v8 2-speaker PIT SNN-TasNet training on Libri2Mix",
...
def main() -> None:
    ...
    # NOTE: dropout is intentionally excluded — v8 uses a new dropout value (0.5)
    # regardless of what the checkpoint stored (v7 saved 0.3).
    if cfg.resume and _peek_ckpt.exists():
        ...
        print(f"[resume] Architecture from checkpoint: "
              ...
              f"decoder_refine={cfg.decoder_refine}  dropout={cfg.dropout} (v8)")
        del _early, _saved
    ...
    print(f"\n[model] SNNTasNet v8  n_speakers={cfg.n_speakers}  params={n_params:,}")
    ...
    # Dropout is a training-time mask — weight shapes are identical to v7, so strict=True.
    ...
    # v8 has freeze_epochs=0: the model is already trained, so the encoder starts
    # unfrozen alongside separator and decoder, but at a lower lr (lr_finetune).
    optimizer = AdamW([
