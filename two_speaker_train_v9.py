"""
two_speaker_train_v9.py — v9 training: 2-speaker PIT SNN-TasNet on Libri2Mix.

Architecture: SNNTasNet v7 (sep_model_v7.py) — unchanged from v7/v8
  Encoder:    Conv1d (1→256, k=32, s=16)
  Separator:  ChunkedSNNSeparator (LIF neurons)
  Decoder:    ConvDecoder (3× Conv1d+GroupNorm+PReLU → ConvTranspose1d)

v9 changes vs v8:
  - dropout 0.5 → 0.35   (v8's 0.5 over-regularised; 0.35 is the middle ground)
  - gain augmentation     (±6 dB independent random gain per source, recompute mix)
                          Targets speaker-identity memorisation — the root cause of
                          the persistent 3+ dB train-val gap
  ...
  - freeze_epochs = 0    (model already trained end-to-end)

Dataset:      LibriMixDataset — pre-generated mix_clean / s1 / s2 wav files
              train-100 (13,900 pairs) + dev (3,000 pairs) on ARC BeeGFS

Resume:       --resume loads checkpoints_v9/best_2spk_latest.pt
SLURM:        slurm_v9_twospeaker.sh (self-resubmitting, 6h wall time)

Usage (smoke test — local, if librimix data present):
    python3 two_speaker_train_v9.py --n_epochs 2 --batch_size 4 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v9_twospeaker.sh
"""
...
from sep_model_v7        import SNNTasNet
...
DEFAULTS = dict(
    ...
    val_split         = "dev",
    clip_len          = DEFAULT_CLIP_LEN,   # 64,000 samples = 4s @ 16kHz
    # ── model ─────────────────────────────────────────────────────────────────
    n_filters         = 256,
    kernel_sz         = 32,
    stride            = 16,
    ...
    n_layers          = 3,
    n_speakers        = 2,
    dropout           = 0.35,   # v9: between v7's 0.3 (too low) and v8's 0.5 (too high)
    snn_chunk         = 100,
    decoder_refine    = 3,
    decoder_groups    = 8,
    # ── training ──────────────────────────────────────────────────────────────
    ...
    seed              = 42,
    # ── augmentation ──────────────────────────────────────────────────────────
    gain_aug_db       = 6.0,    # v9: ±N dB independent random gain per source
    # ── loss ──────────────────────────────────────────────────────────────────
    lambda_spec           = 0.5,
    lambda_rate           = 0.10,
    lambda_recon          = 5.0,
...
def apply_gain_aug(
    ...
    """
    Independent random gain per source, recompute mixture.

    Each source is scaled by 10^(U(-gain_db, +gain_db) / 20).
    Mixture is recomputed so mix = g1*s1 + g2*s2, preserving mix=s1+s2 structure
    but varying per-source SNR. This prevents the separator from exploiting
    loudness cues tied to a specific speaker's training distribution.

    Returns: (mix_aug, t1_aug, t2_aug)  — all (B, T)
    """
    B = t1.shape[0]
    g1_db = t1.new_empty(B, 1).uniform_(-gain_db, gain_db)
    g2_db = t2.new_empty(B, 1).uniform_(-gain_db, gain_db)
    g1 = 10.0 ** (g1_db / 20.0)
    g2 = 10.0 ** (g2_db / 20.0)
    t1_aug = t1 * g1
    t2_aug = t2 * g2
...
def _si_sdr(est: torch.Tensor, ref: torch.Tensor) -> torch.Tensor:
    """SI-SDR per sample. Inputs: (B, T). Returns: (B,)."""
    eps  = 1e-8
...
def _mr_spectral_loss(
    ...
    """Multi-resolution L1 on log-magnitude STFT. Inputs: (B, T). Returns: scalar."""
...
def _spike_rate_loss(
    ...
    [target_min, target_max]. Zero inside the band, quadratic outside.
    """
...
def _recon_loss(estimates: torch.Tensor, mixture: torch.Tensor) -> torch.Tensor:
    """
    RMS-normalised MSE energy conservation loss.
...
def pit_forward(
    ...
    """
    PIT forward pass for 2 speakers.

    Tries both assignments (est[:,0]→t1 or est[:,0]→t2) and keeps the one with
    lower mean negative SI-SDR per sample. torch.min propagates gradients only
    through the winning permutation.
    ...
    # PIT: per-sample, select best permutation via negative SI-SDR
    ...
    # Gather estimates aligned to (t1, t2) under the winning permutation
    idx_t1   = best_perm.view(B, 1, 1).expand(B, 1, T).long()
    idx_t2   = (1 - best_perm).view(B, 1, 1).expand(B, 1, T).long()
    est_t1   = est.gather(1, idx_t1).squeeze(1)    # (B, T)
    est_t2   = est.gather(1, idx_t2).squeeze(1)    # (B, T)
    ...
    rate_t = (rate_loss if isinstance(rate_loss, torch.Tensor)
...
def run_epoch(
    ...
    for batch_idx, (mixture, t1, t2) in enumerate(loader):
        ...
        # Gain augmentation — training only, not val
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v9 2-speaker PIT SNN-TasNet training on Libri2Mix",
...
def main() -> None:
    ...
    print(f"[aug]  gain_aug_db=±{cfg.gain_aug_db} dB (train only)")
    ...
    # dropout intentionally excluded — v9 uses 0.35 regardless of checkpoint value
    ...
    print(f"\n[model] SNNTasNet v9  n_speakers={cfg.n_speakers}  params={n_params:,}")
    print(f"        hidden={cfg.hidden}  n_layers={cfg.n_layers}  "
          f"decoder_refine={cfg.decoder_refine}  dropout={cfg.dropout}")
    print(f"        chunk={cfg.snn_chunk} frames ({bptt_s:.2f}s BPTT window)")

    # ── Load v7 best weights as starting point ────────────────────────────────
    # v7 best (val=-1.16 dB) is used because v8 best (-1.65 dB) regressed.
    # Dropout is a training-time mask — weight shapes are identical, strict=True is safe.
    starter_path = Path(cfg.starter_ckpt)
    if starter_path.exists() and not cfg.resume:
        print(f"\n[starter] Loading v7 best weights from {starter_path}")
        pt    = torch.load(starter_path, map_location=device, weights_only=False)
        state = pt.get("model_state", pt) if isinstance(pt, dict) else pt
        missing, unexpected = model.load_state_dict(state, strict=True)
        print(f"[starter] All weights loaded — continuing from v7 best (val=-1.16 dB)")
        del pt, state
    elif not cfg.resume:
        print(f"\n[warn] No starter checkpoint at {starter_path}. "
              "Training all parameters from scratch.")

    # ── Optimizer — three-group AdamW from epoch 1 (no freeze phase) ─────────
    optimizer = AdamW([
        {"params": model.separator.parameters(), "lr": cfg.lr},
        {"params": model.decoder.parameters(),   "lr": cfg.lr},
        {"params": model.encoder.parameters(),   "lr": cfg.lr_finetune},
    ], weight_decay=cfg.weight_decay)
    print(f"\n[optim] Three-group AdamW  sep/dec lr={cfg.lr:.1e}  enc lr={cfg.lr_finetune:.1e}")
    ...
    for epoch in range(start_epoch, cfg.n_epochs + 1):
        ...
        print(f"{epoch:>5}/{cfg.n_epochs}  {'finetune':>7}  "
        ...
        # ── Checkpoint: best_2spk_latest.pt (every epoch for SLURM resume) ───
        ...
        # ── Early silent-output diagnostic ───────────────────────────────────
