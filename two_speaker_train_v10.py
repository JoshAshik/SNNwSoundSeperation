"""
two_speaker_train_v10.py — v10 training: stateful SNN-TasNet on Libri2Mix.

Architecture: SNNTasNet v10 (sep_model_v10.py)
  Encoder:    Conv1d (1→256, k=32, s=16)           [transferred from v7 best]
  Separator:  StatefulSNNSeparator                  [NEW — freshly initialised]
                hidden=512, n_layers=6, chunk=100
                LIF state persists across the full 4 s clip (40 sequential chunks)
  Decoder:    ConvDecoder (3× Conv1d+GN+PReLU → ConvTranspose1d)
                                                    [transferred from v7 best]

v10 changes vs v9:
  - StatefulSNNSeparator replaces ChunkedSNNSeparator
      LIF neurons carry membrane state across all 40 non-overlapping chunks
      → coherent speaker timeline vs 0.1 s blind windows in v7-v9
  - hidden 256 → 512, n_layers 3 → 6  (separator capacity 4× larger)
  - freeze_epochs = 20  (fresh separator needs warm-up before encoder moves)
  - gain_aug_db = 3.0  (reduced from v9's 6.0 — dataset already has ±6 dB global aug)
  - Partial weight transfer: encoder + decoder from checkpoints_v7/best_2spk.pt
      Separator freshly initialised (shapes changed: 256→512, 3→6 layers)
  - LR reset to 1.5e-4, cosine over finetune phase (ep 21–200)

Performance note:
  StatefulSNNSeparator processes chunks sequentially (batch=B=8 per step instead of
  B*n_segs=640). Epochs will be 3–5× slower than v7/v9 (~3000–5000 s/epoch).
  The SLURM script is self-resubmitting; ~40–50 jobs cover 200 epochs.

Dataset:   LibriMixDataset — pre-generated Libri2Mix wav16k/max/
           train-100 (13,900 pairs) + dev (3,000 pairs) on ARC BeeGFS
           spike_safe_augment already applied in the dataset (global gain + polarity
           + time shift + noise). Training loop adds mild per-source gain (±3 dB).

Resume:    --resume loads checkpoints_v10/best_2spk_latest.pt
SLURM:     slurm_v10_twospeaker.sh (self-resubmitting, 6 h wall)

Usage (smoke test):
    python3 two_speaker_train_v10.py --n_epochs 2 --batch_size 2 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v10_twospeaker.sh
"""
...
DEFAULTS = dict(
    ...
    kernel_sz         = 32,
    stride            = 16,
    hidden            = 512,    # v10: up from v7/v8/v9's 256
    n_layers          = 6,      # v10: up from v7/v8/v9's 3
    n_speakers        = 2,
    dropout           = 0.35,
    snn_chunk         = 100,
    decoder_refine    = 3,
    ...
    lr_finetune       = 5e-5,
    weight_decay      = 1e-4,
    freeze_epochs     = 20,     # v10: freeze encoder while fresh separator warms up
    val_every         = 5,
    grad_clip         = 5.0,
    ema_decay         = 0.999,
    seed              = 42,
    # ── augmentation ──────────────────────────────────────────────────────────
    gain_aug_db       = 3.0,    # v10: per-source ±3 dB (reduced from v9's 6.0)
                                #   dataset already applies global ±6 dB via spike_safe_augment
    # ── loss ──────────────────────────────────────────────────────────────────
    lambda_spec           = 0.5,
    lambda_rate           = 0.10,
    lambda_recon          = 5.0,
...
def apply_gain_aug(
    ...
    Each source scaled by 10^(U(-gain_db, +gain_db) / 20).
    Prevents the separator exploiting loudness as a speaker-identity proxy.
    Applied AFTER the dataset's global gain jitter (spike_safe_augment).
    """
    B = t1.shape[0]
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v10 stateful SNN-TasNet training on Libri2Mix",
...
def main() -> None:
    ...
    if device.type == "cpu":
        print("[warn] No GPU — SNN training on CPU will be very slow")
    ...
    print(f"[aug]  per-source gain_aug_db=±{cfg.gain_aug_db} dB (train only, on top of dataset aug)")
    ...
    if cfg.resume and _peek_ckpt.exists():
        ...
        print(f"[resume] Architecture: hidden={cfg.hidden}  n_layers={cfg.n_layers}  "
              f"dropout={cfg.dropout}  snn_mode=snn_stateful")
        del _early, _saved
    ...
    print(f"\n[model] SNNTasNet v10  params={n_params:,}")
    ...
    # Encoder starts frozen (freeze_epochs); LR group is set to 0 and
    # requires_grad is disabled. At epoch freeze_epochs+1, both are restored.
    optimizer = AdamW([
    ...
    # Cosine schedule covers only the finetune phase (ep freeze+1 … n_epochs)
    ...
    # ── Resume ────────────────────────────────────────────────────────────────
    # Must come BEFORE starter loading and EMA init so _actually_resumed is set
    # correctly. SLURM always passes --resume; the flag only matters when the
    # checkpoint file actually exists.
    start_epoch       = 1
    best_val          = -1e9
    ...
    # Separator dims changed (256→512, 3→6 layers) — only encoder + decoder transfer.
    ...
    if not _actually_resumed:
        if starter_path.exists():
            ...
            v10_state = model.state_dict()
            copied = 0
            for k, v in v7_state.items():
                if k.startswith("encoder.") or k.startswith("decoder."):
                    if k in v10_state and v10_state[k].shape == v.shape:
                        v10_state[k] = v
                        copied += 1
            model.load_state_dict(v10_state)
    ...
    # ── EMA (init after weights are loaded so shadow starts from correct values)
    ...
    # Conditioned on start_epoch, not cfg.resume, so it fires correctly on both
    # fresh starts (epoch 1 ≤ freeze_epochs) and mid-freeze resumes.
