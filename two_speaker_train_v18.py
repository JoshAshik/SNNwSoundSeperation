"""
two_speaker_train_v18.py — v17 recipe + wider separator bottleneck (Experiment F).

v17 (finer encoder k=16/s=8, from scratch, pure SI-SDR, plateau LR) jumped to
+9.35 dB val — confirming the coarse front-end was the ceiling. The train-val gap
stayed ~0 dB (still underfitting, not overfitting), so the indicated next lever is
SEPARATOR CAPACITY. v18 changes exactly ONE variable vs v17:

  F    dprnn_bn_dim 64 -> 128 (separator bottleneck width).
       v14 widened rnn_hidden behind the COARSE encoder and got nothing — that was
       the wrong precondition. With the finer front-end now in place and the model
       still underfitting, adding bottleneck capacity is the textbook move.

Everything else is held identical to v17 for a clean single-variable read:
  - Encoder k=16, s=8; from scratch; no warmstart, no freeze.
  - Pure SI-SDR loss (lambda_recon=0, lambda_spec=0.1, lambda_rate=0).
  - Fixed REAL Libri2Mix train-100; DATASET augmentation OFF. (NOTE: the encoder's
    SpecAugment freq-masking still fires during training — sep_model_v10.py:687,
    gated on model.training. It is held identical to v17, so the v17->v18 comparison
    stays single-variable; turning SpecAugment off is a separate future experiment.)
  - Adam lr=1e-3 + ReduceLROnPlateau (ABS 0.01 dB threshold), grad clip 5.

NOTE the wider bottleneck enlarges every DPRNN block (intra/inter BiLSTM input dim,
out_proj). On an A100 this is comfortably within memory — batch_size can stay 8 or
go higher (see slurm_v18_twospeaker.sh). rnn_hidden stays 128 (only bn_dim moves).

Alternative (faster, NOT default): warmstart encoder+decoder from v17 and train a
fresh wider separator. Not wired in here to keep the single-variable comparison to
v17 clean; v17's encoder is identical-shape and could be transferred if desired.
...
SLURM:     slurm_v18_twospeaker.sh (self-resubmitting, ~6 h wall).

Usage (smoke test):
    python3 two_speaker_train_v18.py --n_epochs 2 --batch_size 2 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v18_twospeaker.sh
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Defaults  (v18: v17 recipe + wider separator bottleneck bn_dim=128)
# ─────────────────────────────────────────────────────────────────────────────
...
DEFAULTS = dict(
    ...
    decoder_refine    = 3,
    decoder_groups    = 8,
    dprnn_bn_dim      = 128,       # F: WIDENED from 64 (the single v18 variable)
    dprnn_rnn_hidden  = 128,       # held at v17 size — only bn_dim moves
...
def _hann_window(n_fft: int, device: torch.device) -> torch.Tensor:
    """Cache Hann windows by (n_fft, device) — rebuilt every batch otherwise."""
...
def _mr_spectral_loss(est: torch.Tensor, ref: torch.Tensor,
    ...
    # STFT has no bf16/complex-bf16 kernel — force fp32 and disable autocast so
    # this is safe under either fp16 or bf16 mixed precision.
    with autocast("cuda", enabled=False):
        ...
        for n_fft in fft_sizes:
            hop = n_fft // 4
            win = _hann_window(n_fft, est.device)      # cached — avoids per-call alloc
...
def pit_forward(model: SNNTasNet, mixture: torch.Tensor,
    ...
    # Optional small perceptual term (lambda_spec defaults to 0.1 in v18).
    spec_val = total.new_zeros(())
    ...
    # Metrics are returned as detached fp32 0-dim tensors and accumulated on-device
    # in run_epoch; .item() (the device→host sync) happens only at print/epoch-end,
    # not every batch — keeps the A100 pipeline from stalling each step.
    metrics = {
        "loss":       total.detach().float(),
        "si_sdri":    si_sdri_val.float(),
        "spec_loss":  spec_val.float(),
        "wform_mean": est.detach().abs().mean().float(),
    }
...
def run_epoch(model, loader, optimizer, device, cfg,
    ...
    for batch_idx, (mixture, t1, t2) in enumerate(loader):
        ...
        # bf16 autocast on A100: same dynamic range as fp32, so no GradScaler /
        # loss-scaling machinery is needed. set_grad_enabled(is_train) disables the
        # backward graph on val (6 DPRNN blocks × BiLSTMs over ~8000 frames) — large
        # VRAM/time saving; the metric is identical either way.
        ...
        # Accumulate on-device (no sync). metrics values are detached 0-dim tensors.
    ...
    # Single device→host sync per metric, at epoch end (not per batch).
    out = {}
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v18 finer encoder + wider bottleneck (bn_dim=128), from scratch",
...
def main() -> None:
    ...
    print(f"\n[model] SNNTasNet v18 (finer encoder k={cfg.kernel_sz}/s={cfg.stride}, "
          f"bn_dim={cfg.dprnn_bn_dim}, from scratch)  params={n_params:,}  "
    ...
    print(f"[amp]   autocast=bfloat16 (no GradScaler)  on {device.type}")
