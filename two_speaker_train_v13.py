"""
two_speaker_train_v13.py — v13 DPRNN separator: architecture upgrade from v12.

Architecture: SNNTasNet v13 (sep_model_v10.py, snn_mode=dprnn)
  Replaces StatefulGRUSeparator with DPRNNSeparator (Dual-Path RNN).
  Encoder and ConvDecoder unchanged — warm-started from v12a best.

v13 changes vs v12:
  - DPRNNSeparator: dual-path BiLSTM, processes full sequence at once
  - bn_dim=64, rnn_hidden=128, 6 blocks, chunk_size=200
  - lr = 5e-4 (fresh separator), freeze_epochs=20 (protect encoder)
  - n_epochs = 200, dropout = 0.1
  - gain_aug_db = 3.0, train_augment = True (fresh training)
  - Warmstart: encoder+decoder only from v12a (separator keys filtered)

Dataset:   LibriMixDataset — pre-generated Libri2Mix wav16k/max/
Resume:    --resume loads checkpoints_v13/best_2spk_latest.pt
Warmstart: --warmstart (default: checkpoints_v12a/best_2spk.pt)
SLURM:     slurm_v13_twospeaker.sh (self-resubmitting, 6 h wall)

Usage (smoke test):
    python3 two_speaker_train_v13.py --n_epochs 2 --batch_size 2 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v13_twospeaker.sh
"""
...
DEFAULTS = dict(
    ...
    kernel_sz         = 32,
    stride            = 16,
    hidden            = 128,       # maps to rnn_hidden for DPRNN
    n_layers          = 6,         # maps to n_blocks for DPRNN
    n_speakers        = 2,
    dropout           = 0.1,
    ...
    num_workers       = 8,
    lr                = 5e-4,
    lr_finetune       = 5e-5,      # encoder+decoder LR after freeze
    weight_decay      = 1e-4,
    freeze_epochs     = 20,
    val_every         = 5,
    grad_clip         = 5.0,
    ...
    # ── paths ─────────────────────────────────────────────────────────────────
    warmstart         = "./checkpoints_v12a/best_2spk.pt",
    starter_ckpt      = "./checkpoints_v12a/best_2spk.pt",
    ckpt_dir          = "./checkpoints_v13",
    log_dir           = "./runs_v13",
    csv_path          = "./v13_train_log.csv",
    # ── runtime ───────────────────────────────────────────────────────────────
    resume            = False,
    max_wall_hours    = 0.0,
)
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v13 DPRNN separator: architecture upgrade from v12",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--resume",          action="store_true")
    p.add_argument("--warmstart",       type=str, default=defaults["warmstart"],
                   help="Load encoder+decoder weights (EMA-preferred) from this "
                        "checkpoint. Separator keys are filtered out.")
...
def main() -> None:
    ...
    print(f"\n[model] SNNTasNet v13 (DPRNN)  params={n_params:,}  "
    ...
    if cfg.resume and _peek_ckpt.exists():
        ...
        # restore freeze state and rebuild optimizer to match
        ...
        # load optimizer/scheduler state only if param groups match
    ...
    # ── Warmstart: encoder+decoder from v12a, fresh separator ────────────────
    if cfg.warmstart and not _actually_resumed:
        ...
        if ws_path.exists():
            ...
            print(f"[warmstart] Separator: freshly initialised (DPRNNSeparator)")
            del ws_ckpt
        else:
