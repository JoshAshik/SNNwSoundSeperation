"""
two_speaker_train_v14.py — v14 wider DPRNN: rnn_hidden 128→256.

Architecture: SNNTasNet v14 (sep_model_v10.py, snn_mode=dprnn)
  Same DPRNNSeparator as v13 but with rnn_hidden=256 (was 128).
  Encoder and ConvDecoder warm-started from v13 best (not v12a).

v14 changes vs v13:
  - dprnn_rnn_hidden = 256 (was 128) — wider BiLSTM per block
  - batch_size = 16 (was 8) — A100 has headroom
  - warmstart from v13 best (encoder+decoder co-adapted with DPRNN masks)
  - separator keys filtered (rnn_hidden shape changed)

Dataset:   LibriMixDataset — pre-generated Libri2Mix wav16k/max/
Resume:    --resume loads checkpoints_v14/best_2spk_latest.pt
Warmstart: --warmstart (default: checkpoints_v13/best_2spk.pt)
SLURM:     slurm_v14_twospeaker.sh (self-resubmitting, 6 h wall)

Usage (smoke test):
    python3 two_speaker_train_v14.py --n_epochs 2 --batch_size 2 --num_workers 0 \\
        --librimix_root ./data/librimix --max_wall_hours 0

Usage (ARC):
    sbatch slurm_v14_twospeaker.sh
"""
...
DEFAULTS = dict(
    ...
    kernel_sz         = 32,
    stride            = 16,
    hidden            = 256,       # maps to rnn_hidden for DPRNN (was 128 in v13)
    ...
    decoder_groups    = 8,
    dprnn_bn_dim      = 64,
    dprnn_rnn_hidden  = 256,       # was 128 in v13
    # ── training ──────────────────────────────────────────────────────────────
    n_epochs          = 200,
    batch_size        = 16,        # was 8 in v13 — A100 has headroom
    num_workers       = 8,
    lr                = 5e-4,
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v14 wider DPRNN: rnn_hidden 128→256",
...
def main() -> None:
    ...
    print(f"\n[model] SNNTasNet v14 (wider DPRNN)  params={n_params:,}  "
    ...
    if cfg.warmstart and not _actually_resumed:
        ...
        if ws_path.exists():
            ...
            if "ema_state" in ws_ckpt and ws_ckpt["ema_state"].get("shadow"):
                ...
                print(f"[warmstart] Skipped {skipped} separator params (fresh wider DPRNN)")
            else:
            ...
            print(f"[warmstart] Separator: freshly initialised (wider DPRNNSeparator, rnn_hidden={cfg.dprnn_rnn_hidden})")
            del ws_ckpt
        else:
