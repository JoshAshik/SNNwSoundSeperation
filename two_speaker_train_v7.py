"""
two_speaker_train_v7.py — v7 training: 2-speaker PIT SNN-TasNet on Libri2Mix.

Architecture: SNNTasNet v7 (sep_model_v7.py)
  Encoder:    Conv1d (1→256, k=32, s=16)           — pre-trained weights from v6
  Separator:  ChunkedSNNSeparator (LIF neurons)     — fresh
  Decoder:    ConvDecoder (3× Conv1d+GroupNorm+PReLU → ConvTranspose1d) — fresh + upsample from v6

Dataset:      LibriMixDataset — pre-generated mix_clean / s1 / s2 wav files
              train-100 (13,900 pairs) + dev (3,000 pairs) on ARC BeeGFS

Starting checkpoint: v7_starter.pt
  Encoder weights transferred from checkpoints_v6/best_2spk.pt.
  Separator and ConvDecoder refine blocks start fresh.
  Load with model.load_state_dict(..., strict=False) — no transfer_encoder_decoder needed.

Freeze strategy (v7):
  Epochs 1–freeze_epochs: encoder frozen, separator + full decoder train freely.
    (v6 also froze the decoder, but in v7 the ConvDecoder refine blocks are new capacity
     that must learn from epoch 1 — freezing them would prevent the separator from seeing
     useful decoder gradients during the warm-up phase.)
  Epochs freeze_epochs+1 onward: encoder unfrozen at lr_finetune (lower), everything else at lr.

Resume:       --resume loads checkpoints_v7/best_2spk_latest.pt
SLURM:        slurm_v7_twospeaker.sh (self-resubmitting, 6h wall time)
...
DEFAULTS = dict(
    ...
    lambda_spec           = 0.5,
    lambda_rate           = 0.10,
    lambda_recon          = 5.0,    # conservative start vs v6's 10.0 — Libri2Mix
                                    # sources are clean speech, less decoder amplification
    target_spike_rate     = 0.10,
    target_spike_rate_max = 0.30,
    # ── paths ─────────────────────────────────────────────────────────────────
    starter_ckpt      = "./v7_starter.pt",
    ckpt_dir          = "./checkpoints_v7",
    log_dir           = "./runs_v7",
    csv_path          = "./v7_train_log.csv",
    # ── runtime ───────────────────────────────────────────────────────────────
    resume            = False,
    max_wall_hours    = 0.0,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# Freeze helpers  (v7: freeze encoder only — decoder refine blocks must train)
# ─────────────────────────────────────────────────────────────────────────────

def _freeze_encoder(model: SNNTasNet) -> None:
...
def _parse_args(defaults: dict) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="v7 2-speaker PIT SNN-TasNet training on Libri2Mix",
...
def main() -> None:
    ...
    if cfg.resume and _peek_ckpt.exists():
        ...
        print(f"[resume] Architecture from checkpoint: "
              ...
              f"decoder_refine={cfg.decoder_refine}")
        del _early, _saved
    ...
    print(f"\n[model] SNNTasNet v7  n_speakers={cfg.n_speakers}  params={n_params:,}")
    print(f"        hidden={cfg.hidden}  n_layers={cfg.n_layers}  "
          f"decoder_refine={cfg.decoder_refine}")
    print(f"        chunk={cfg.snn_chunk} frames ({bptt_s:.2f}s BPTT window)")

    # ── Load v7 starter weights (encoder from v6 transfer) ───────────────────
    starter_path = Path(cfg.starter_ckpt)
    if starter_path.exists() and not cfg.resume:
        print(f"\n[starter] Loading v7 starter weights from {starter_path}")
        pt  = torch.load(starter_path, map_location=device, weights_only=False)
        # Handle both raw state_dict and checkpoint-format saves
        state = pt.get("model_state", pt) if isinstance(pt, dict) else pt
        missing, unexpected = model.load_state_dict(state, strict=False)
        if missing:
            print(f"[starter] {len(missing)} missing keys (expected for fresh separator/decoder)")
        if unexpected:
            print(f"[starter] {len(unexpected)} unexpected keys")
        print("[starter] Encoder weights loaded — separator+decoder refine blocks start fresh")
        del pt, state
    elif not cfg.resume:
        print(f"\n[warn] No starter checkpoint at {starter_path}. "
              "Training all parameters from scratch — slower convergence expected.")

    # ── Determine freeze phase from checkpoint epoch ──────────────────────────
    ...
    if unfrozen:
        ...
        print(f"[optim] Three-group AdamW (finetune phase restored)  "
              f"sep/dec lr={cfg.lr:.1e}  enc lr={cfg.lr_finetune:.1e}")
    else:
    ...
    for epoch in range(start_epoch, cfg.n_epochs + 1):
        ...
        if epoch == cfg.freeze_epochs + 1 and not unfrozen:
            print(f"\n  [epoch {epoch}] Unfreezing encoder  lr_finetune={cfg.lr_finetune:.1e}")
            _unfreeze_all(model)
            optimizer = AdamW([
                {"params": model.separator.parameters(), "lr": cfg.lr},
                {"params": model.decoder.parameters(),   "lr": cfg.lr},
